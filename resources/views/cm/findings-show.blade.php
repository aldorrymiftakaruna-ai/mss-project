        ...
        <form action="{{ route('cm.findings.update', $cmFinding) }}" method="POST" enctype="multipart/form-data" class="px-6 py-5 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <textarea name="action" rows="2" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">{{ $cmFinding->action }}</textarea>
                </div>
            </div>

            <div>
                <label class="text-xs text-gray-500 mb-1 block">Date Action</label>
                <input type="date" name="date_action" value="{{ $cmFinding->date_action ? $cmFinding->date_action->format('Y-m-d') : '' }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
            </div>
